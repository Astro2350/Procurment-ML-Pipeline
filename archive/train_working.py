...
# -----------------------------
cat_vocab_sizes = [
    int(df['hospital_system_id_encoded'].max()) + 1,
    int(df['primary_category_id_encoded'].max()) + 1,
    int(df['department_name_encoded'].max()) + 1
]
hier_vocab_sizes = [int(df[f'category{i}_encoded'].max()) + 1 for i in range(6)]
...
class TransactionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Use amount if available; otherwise default to 0.0
...
for epoch in range(num_epochs):
    ...
    if val_f1 > best_f1:
        ...
        torch.save(best_model_state, os.path.join(experiment_dir, "best_model.pth"))
